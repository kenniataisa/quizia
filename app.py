...
# ======================
# FUNÇÕES CORE (Copie as suas aqui)
# ======================
...
def save_questions_to_db(quiz_id, questions_data):
    ...
    try:
        ...
        # Adicionando um retorno para saber se a operação teve sucesso
        return True
    except Exception as e:
...
if menu == "Gerar Novo Quiz":
    ...
    st.markdown("O conteúdo do PDF será processado para criar um quiz completo que ficará disponível para todos.")

    with st.container(border=True):
        uploaded_file = st.file_uploader("1. Selecione o arquivo PDF", type=["pdf"])
        
        # *** NOVO CAMPO PARA RENOMEAR O QUIZ ***
        quiz_name = st.text_input(
            "2. Dê um nome para o seu Quiz", 
            value=uploaded_file.name if uploaded_file else ""
        )
        
        col1, col2 = st.columns(2)
        with col1:
            dificuldade = st.selectbox("3. Dificuldade das Questões", ["Fácil", "Médio", "Difícil", "Aleatório"])
        with col2:
            estilo = st.selectbox("4. Estilo das Questões", ["Múltipla escolha", "Verdadeiro/Falso", "Aleatório"])
        
        if st.button("Analisar e Gerar Quiz Completo", type="primary", disabled=(not client or not uploaded_file or not quiz_name)):
            with st.status("A iniciar o processo de geração do quiz...", expanded=True) as status:
                try:
                    ...
                    status.update(label="Passo 2/5: A dividir o conteúdo...", state="running")
                    chunks = chunk_text(pdf_text)
                    num_chunks = len(chunks)
                    st.write(f"Conteúdo dividido em {num_chunks} partes para análise.")
                    
                    # *** USANDO O NOME PERSONALIZADO ***
                    status.update(label="Passo 3/5: A criar registo do quiz...", state="running")
                    ...
                    status.update(label="Passo 5/5: A salvar questões no banco de dados...", state="running")
    ...
    if not quizzes:
        ...
        if quiz_id_selecionado:
            if "quiz_id_selecionado" not in st.session_state or st.session_state.quiz_id_selecionado != quiz_id_selecionado:
                ...
                # *** ADICIONADO DEBUGGING ***
                st.write(f"🔎 Buscando questões para o Quiz ID: {quiz_id_selecionado}")
                dados_recebidos = get_questions_for_quiz(quiz_id_selecionado)
                st.write(f"📦 Dados recebidos do Supabase: {dados_recebidos}")
                
                st.session_state.quiz_data = dados_recebidos
            ...
            if st.session_state.get("quiz_data"):
                # O resto do seu código para exibir as questões...
                pass # (O código original para exibir e responder a questão continua aqui)
